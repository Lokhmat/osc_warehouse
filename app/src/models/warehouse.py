...
class SimpleWarehouse(BaseModel):
    ...
    def get_warehouse_model(self) -> Warehouse:
        return Warehouse(
            id=str(uuid.uuid4()),
...
def create_warehouse(engine, warehouse: SimpleWarehouse) -> Warehouse:
    with engine.connect() as connection:
        with open(f'{BASE_POSTGRES_TRANSACTIONS_DIRECTORY}/warehouse/create_warehouse.sql') as sql:
            query = text(sql.read())
            args = warehouse.get_warehouse_model().model_dump()
...
def get_warehouse_by_id(engine, id: str) -> typing.Optional[Warehouse]:
    ...
    with engine.connect() as connection:
        with open(f'{BASE_POSTGRES_TRANSACTIONS_DIRECTORY}/warehouse/get_warehouse_by_id.sql') as sql:
            query = text(sql.read())
...
def update_warehouse(engine, warehouse_update: WarehouseUpdate) -> Warehouse:
    with engine.connect() as connection:
        with open(f'{BASE_POSTGRES_TRANSACTIONS_DIRECTORY}/warehouse/update_warehouse.sql') as sql:
            query = text(sql.read())
            args = warehouse_update.model_dump()
            result = connection.execute(query, args).all()
            if not result:
                raise RuntimeError('Failed to update warehouse')
            connection.commit()
...
def delete_warehouse(engine, id: str) -> None:
    with engine.connect() as connection:
        with open(f'{BASE_POSTGRES_TRANSACTIONS_DIRECTORY}/warehouse/delete_warehouse.sql') as sql:
            query = text(sql.read())
